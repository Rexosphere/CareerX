...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <x-button label="{{ $showForm ? 'Cancel' : 'Add Research Project' }}"
            icon="o-{{ $showForm ? 'x-mark' : 'plus' }}" class="btn-primary" wire:click="toggleForm" />
    </div>
            ...
            <div class="card-body">
                ...
                <form wire:submit="save" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <x-select label="Project Type *" wire:model="type" icon="o-document-text" :options="[
                ['id' => 'Undergraduate Project', 'name' => 'Undergraduate Project'],
                ['id' => 'Msc Thesis', 'name' => 'Msc Thesis'],
                ['id' => 'PhD Research', 'name' => 'PhD Research'],
                ['id' => 'Research Paper', 'name' => 'Research Paper'],
                ['id' => 'Conference Paper', 'name' => 'Conference Paper'],
                ['id' => 'Journal Article', 'name' => 'Journal Article'],
            ]" option-value="id" option-label="name" />

                        <x-select label="Status *" wire:model="status" icon="o-chart-bar" :options="[
                ...
                ['id' => 'Published', 'name' => 'Published'],
                            ...
                            <div class="flex flex-wrap gap-2 mt-2">
                                @foreach($authors as $index => $author)
                                    <div class="badge badge-lg gap-2">
                                        {{ $author }}
                                        <button type="button" wire:click="removeAuthor({{ $index }})" class="btn btn-ghost btn-xs">
                            ...
                            <div class="flex flex-wrap gap-2 mt-2">
                                @foreach($tags as $index => $tag)
                                    <div class="badge badge-primary badge-outline gap-2">
                                        {{ $tag }}
                                        <button type="button" wire:click="removeTag({{ $index }})" class="btn btn-ghost btn-xs">
                                            <x-icon name="o-x-mark" class="w-3 h-3" />
                                        </button>
                                    </div>
                                @endforeach
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <x-input label="DOI" wire:model="doi" icon="o-hashtag" placeholder="e.g., 10.1000/xyz123" />

                        <x-input label="Conference" wire:model="conference" icon="o-presentation-chart-line"
                            placeholder="Conference name" />

                        <x-input label="Journal" wire:model="journal" icon="o-book-open" placeholder="Journal name" />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" wire:model="file" accept=".pdf" class="file-input file-input-bordered w-full" />
                        @if($file)
                    ...
                    <div class="flex justify-end gap-2 pt-4">
                        ...
                        <x-button label="{{ $editMode ? 'Update' : 'Save' }} Project" class="btn-primary" type="submit"
                            spinner="save" />
                    </div>
                </form>
            </div>
                    ...
                    <div class="card-body">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <h3 class="card-title text-xl">
                                    {{ $project->title }}
                                    <div
                                        class="badge badge-{{ $project->status === 'Published' ? 'success' : ($project->status === 'Completed' ? 'info' : 'warning') }}">
                                        {{ $project->status }}
                                    </div>
                                </h3>
                            ...
                            <div class="flex gap-2">
                                <button wire:click="edit({{ $project->id }})" class="btn btn-ghost btn-sm">
                                    <x-icon name="o-pencil" class="w-4 h-4" />
                                </button>
                                <button wire:click="delete({{ $project->id }})" class="btn btn-ghost btn-sm text-error"
                        ...
                        <div class="flex flex-wrap gap-2 mt-4">
                            @if($project->publication_link)
                                <a href="{{ $project->publication_link }}" target="_blank" class="btn btn-sm btn-outline gap-2">
                                    <x-icon name="o-link" class="w-4 h-4" />
                                    Publication
                                </a>
                            @endif
                            @if($project->doi)
                                <a href="https://doi.org/{{ $project->doi }}" target="_blank" class="btn btn-sm btn-outline gap-2">
        ...
        @if(!$showForm)
            <div class="card bg-base-100 shadow-xl">
                <div class="card-body text-center py-12">
                    ...
                    <div class="mt-6">
                        <x-button label="Add Your First Project" icon="o-plus" class="btn-primary" wire:click="toggleForm" />
                    </div>
                </div>
            </div>
        @endif
